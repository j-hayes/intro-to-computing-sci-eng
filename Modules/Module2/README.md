...
- Another grouping would be the person's information and the latitude and longitude of a location. These are themselves groupings of multiple different properties represented by simple types (strings for the names and a numbers for the day and the lat/long). These are often called composite types or classes. (Specific instances of these classes are usually called objects) 
